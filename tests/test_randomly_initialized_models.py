...
def test_random_backbone(backbone):
    model = Model(num_channels=3, multi_image=False, backbone=backbone, fpn=False, head=None, head_outputs=None, weights=None)
...
def test_random_backbone_with_fpn(backbone):
    model = Model(num_channels=3, multi_image=False, backbone=backbone, fpn=True, head=None, head_outputs=None, weights=None)
    rand_img = torch.rand((8, 3, 128, 128))
    output = model(rand_img)
    assert output is not None
...
def test_random_backbone_with_fpn_and_head(backbone, head):
    model = Model(num_channels=3, multi_image=False, backbone=backbone, fpn=True, head=head, head_outputs=2, weights=None)
